/*******************************************************************************
purpose: 		evaluate linkage for the Nucleotide RCT
date 
created: 		03/04/2019
...
********************************************************************************
capture 	log close
log 		using "${logdir}\04-ev_linkage_trial_6 $S_DATE.log", replace 
...
********************************************************************************
*                            keep only those from trial 6                      *
********************************************************************************
keep if		trial==6
count if	trial==6 // 332
tab			group, m 
count if	pupilreference==. // 34
count if	studyid1=="" // 0
gen			rct9plausfirstacyr = 2006
...
count if	studyid1=="" // 3,088
drop if		_merge == 2 // some pupilrefs dont have a linked studyid (drop these) 
drop		_merge // but keep unmatched study IDs
count		// 332
count if	pupilreference==. // 34
count if	studyid1=="" // 0
egen		rct6_avg1stacyr = mean(first_acyr)
lab var		rct6_avg1stacyr "RCT 6 average first academic year"


********************************************************************************
*                 are there any study ID duplicates?                           *
********************************************************************************
count if	dup_studyid>0 & dup_studyid !=. // yes there are ----18---- duplicate study IDs
count if	studyid1=="" // 0
...
count if	firstagelink >20 // 5
egen		rct6_avgage1stlink = mean(firstagelink)
lab var		rct6_avgage1stlink "RCT 6 average first age at link"
...
replace		truematch =1 if (fnscore ==1 | fnscore ==2 | fnscore ==3 ) & snscore==3 & (locationscore ==1 | locationscore ==2) & first_acyr >2005 & first_acyr <. // nucleotides
replace		truematch =1 if (fnscore ==1 | fnscore ==2 | fnscore ==3 ) & snscore==3 & dbscore ==1 & first_acyr >2005 & first_acyr <. // nucleotides
replace		truematch =1 if (fnscore ==1 | fnscore ==2 | fnscore ==3 ) & (locationscore ==1 | locationscore ==2) & dbscore ==1 & first_acyr >2005 & first_acyr <. // nucleotides
replace		truematch =1 if snscore==3 & (locationscore ==1 | locationscore ==2) & dbscore ==1 & first_acyr >2005 & first_acyr <. // nucleotides
...
*by academic year
replace		falsematch =1 if first_acyr <2006 
replace		falsematch =1 if last_acyr <2006
...
* save 
********************************************************************************
save		"S:\Head_or_Heart\max\post-trial-extension\1-data\04-ev_linkage_t6.dta", replace
