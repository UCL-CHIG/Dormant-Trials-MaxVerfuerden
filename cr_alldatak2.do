...
*		    KS2 from all data				 	  		  			           *
********************************************************************************
** !! READSCORE AND MATSCORE ARE ONLY AVAILABLE FOR 8 PPL SO NO STANDARDISED SCORES ONLY RAW ONES

preserve
use 		"S:\Head_or_Heart\max\post-trial-extension\1-data\alldata.dta", clear
tempfile	ks2alldata
count		// 1,766,399
keep if 	strpos(dataset, "ks2")>0 | strpos(npdfieldreference, "ks2")>0 | strpos( dataset, "KS2")>0 | strpos(npdfieldreference, "KS2")>0 // (1,448,137 observations deleted)
keep if 	strpos(npdfieldreference, "ERD")>0 | strpos(npdfieldreference, "MTOT")>0 | strpos(npdfieldreference, "ETOT")>0 | strpos(npdfieldreference, "ENTT_FG")>0 | strpos(npdfieldreference, "MATT_FG")>0  // (295,046 observations deleted)

* which variables in KS2 have the most entries?
tab			npdfieldreference, sort
replace		indicatorvalue= trim(indicatorvalue) // (4,070 real changes made)
drop if		indicatorvalue=="" 		// (760 observations deleted)
...
drop if		ks2_mat_fg==. & ks2_engtot_fg==. & ks2_mat_raw==. & ks2_engread_raw==. & ks2_engtot_raw==. // (5,629 observations deleted)
...
drop		if dup>1															// (13,945 observations deleted)
