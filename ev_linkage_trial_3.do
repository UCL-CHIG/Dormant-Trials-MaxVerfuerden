/*******************************************************************************
purpose: 		evaluate linkage for the preterm postdischarge feed RCT
date 
created: 		02/04/2019
...
********************************************************************************
merge 		m:1 studyid using "S:\Head_or_Heart\max\attributes\1-Data\all\attributedataset_randomised.dta", update keepusing(byr bayley_PDI bayley_MDI parity smokdur matage matedu alcdur apgar5m kps_fullscore iq_score centre gestage bwt multiple address_tot age_firstadd age_lastaddgrp age_lastadd fup*)
replace		unmatched =1 if _merge==2
count if	pupilreference==. // 1,569
count if	studyid1=="" // 0
drop		_merge
...
********************************************************************************
*                            keep only those from trial 3                    *
********************************************************************************
keep if		trial==3
count if	trial==3 //314
tab			group, m 
count if	pupilreference==. // 1
count if	studyid1=="" // 0
...
********************************************************************************
count if	dup_studyid>0 & dup_studyid !=. // yes there are ----37---- duplicate study IDs
count if	studyid1=="" // 0
...
count if	dup_pupilid>0 & dup_pupilid<. // there are ----0---- duplicate pupil IDs
...
count if	firstagelink >20 // 1
egen		rct3_avgage1stlink = mean(firstagelink)
lab var		rct3_avgage1stlink "RCT 3 average first age at link"
...
replace		truematch =1 if (fnscore ==1 | fnscore ==2 | fnscore ==3 ) & snscore==3 & (locationscore ==1 | locationscore ==2) &  first_acyr >1997 & first_acyr <2015 // PDP
replace		truematch =1 if (fnscore ==1 | fnscore ==2 | fnscore ==3 ) & snscore==3 & dbscore ==1 & first_acyr >1997 & first_acyr <2015 // PDP
replace		truematch =1 if (fnscore ==1 | fnscore ==2 | fnscore ==3 ) & (locationscore ==1 | locationscore ==2) & dbscore ==1 & first_acyr >1997 & first_acyr <2015 // PDP
replace		truematch =1 if snscore==3 & (locationscore ==1 | locationscore ==2) & dbscore ==1 & first_acyr >1997 & first_acyr <2015 // PDP
...
* save 
********************************************************************************
save		"S:\Head_or_Heart\max\post-trial-extension\1-data\04-ev_linkage_t3.dta", replace
